...
void main() {
  ...
  float F = 0.02 + 0.58 * pow(1.0 - ndv, 5.0);  // 反射权重上限压低(防坡度整块点亮)
  // IBL 反射(rough 0.05 → mip 0.2)
  vec3 refl = textureLod(texPrefilter, reflect(-v, n), 1.2).rgb;
  ...
  if (lu.lightCount.x > 0.5 && lu.lights[0].w < 0.5) {
    ...
    float Fsun = 0.02 + 0.3 * pow(1.0 - ndv, 5.0);  // 高光菲涅尔限幅(坡度上防整块点亮)
    sunSpec = min(lu.lights[2].rgb, vec3(1.2)) * ndl * Fsun * ggxD(ndh, 0.35);
  }
  ...
  float alpha = clamp(0.25 + 0.75 * (0.02 + 0.98 * pow(1.0 - ndv, 5.0)), 0.0, 1.0);
